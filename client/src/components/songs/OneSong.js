...
function OneSong() {
  ...
  const handleLike = async () => {
    try {
      await axios.post(
        "/favorites/likedSong",
        { song: Auth.song, email: Auth.email },
        {
          headers: {
      ...
      if (!liked) {
        const { data } = await axios.post(
          "/favorites/song",
          { email: Auth.email, songId: Auth.song.id },
          {
            headers: {
              token: Cookie.get("token"),
            },
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        Auth.setSong(data);
        if (query.get("artist")) {
          data = await axios.get(`/artists/${query.get("artist")}`, {
            headers: {
              token: Cookie.get("token"),
            },
          });
          ...
          data = await axios.get(`/favorites/songs/${email}`, {
            headers: {
              token: Cookie.get("token"),
            },
          });
          ...
          data = await axios.get(`/albums/${query.get("album")}`, {
            headers: {
              token: Cookie.get("token"),
            },
          });
          ...
          data = await axios.get(`/playlists/${query.get("playlist")}`, {
            headers: {
              token: Cookie.get("token"),
            },
          });
          ...
          data = await axios.get(`/songs/top`, {
            headers: {
              token: Cookie.get("token"),
            },
          });
        ...
        <div className='oneSong'>
          <div className='playOneSong'>
            ...
            <span style={spanStyle}>
              album: &nbsp;{Auth.song.Album["name"]}
            </span>
            <span style={spanStyle}>
              {" "}
              |&nbsp;{Auth.song.length.slice(3, 10)}
            </span>
            <span style={spanStyle}>
              {" "}
              |&nbsp;views: {Auth.song.playCount + 1}
            </span>
            <MyPlayer />
          ...
          <div className='queue'>
            ...
            {Auth.list.map((item) => {
              ...
              return (
                <SongItem
                  background={item.title === Auth.song.title}
                  animation={false}
                  query={Auth.nextQuery}
